...
export function buildSidebarItems(): SidebarItem[] {
  ...
  for (const category of TAB_ORDER) {
    ...
    const categoryFlags = FLAG_DEFINITIONS.filter((f) => f.category === category)
...
export function getSectionBoundaries(): { category: TabCategory; startIndex: number }[] {
...
export function indexToFlag(selectableIndex: number): FlagDefinition | undefined {
